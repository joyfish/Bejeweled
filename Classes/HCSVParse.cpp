...
HCSVParse::~HCSVParse(void)
{
    
}

void HCSVParse::StringSplit( const string& str, vector<string>& tokens, const char& delimiters )
{
...
void HCSVParse::split( vector<string>& field, string line )
{
    string fld;
    unsigned int i,j=0;
...
int HCSVParse::advplain( const string& s, string& fld, int i)
{
    unsigned int j;
...
int HCSVParse::advquoted( const string& s, string& fld, int i)
{
    unsigned int j;
    fld = "";
...
//解析 CVS 文件
bool HCSVParse::openFile( const char* fileName )
{
...
//获取指定行列的数据
const char* HCSVParse::getData(unsigned int rows, unsigned int cols )
{
...
//获取指定数据的列下标
int HCSVParse::findColsData( int cols, const char* value )
{
